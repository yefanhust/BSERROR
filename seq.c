...
const real prob = 0.95; //tolerable probability

//function performing the gaussian integral 
...
int main(int argc, char *argv[])
{
  ...
  for (unsigned long long i = 0; i < M; ++i){
    err = 0.0;
    //generate N gaussian distribution random numbers into NRV
    vdRngGaussian(VSL_RNG_METHOD_GAUSSIAN_BOXMULLER, stream, N, NRV, 0.0f, 1.0f);
    BM[0] = sqrt((real)T/N)*NRV[0];
    for (int j = 1; j < N; ++j){
      BM[j] = BM[j-1] + sqrt((real)T/N)*NRV[j]; //brownian motion
    }

    PX[0] = X0; //initial price 
    for (int j = 1; j < N+1; ++j){
...
//gaussian integral using Simpson's rule 
